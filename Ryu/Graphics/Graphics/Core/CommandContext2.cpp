...
namespace Ryu::Gfx
{
	...
	CommandContext2::~CommandContext2()
	{
		RYU_ASSERT(!m_cmdQueue && !m_fence && !m_cmdList,
			"CommandContext2 is not properly destructed. Call Destroy() first.");
	}

	void CommandContext2::Create(CommandListType type)
	{
		RYU_PROFILE_SCOPE();
