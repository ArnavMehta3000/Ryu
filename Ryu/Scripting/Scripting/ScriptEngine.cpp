...
namespace Ryu::Scripting
{
	...
	ScriptEngine::~ScriptEngine()
	{
		...
		for (auto& [script, ctx] : m_contexts)
		{
			if (ctx)
			{
				RYU_LOG_TRACE("Releasing context for {}", script);
				ctx->Release();
			}
		}
	...
	void ScriptEngine::EngineLogMessageCallback(asSMessageInfo* msg, void* param)
	{
		...
		case asMSGTYPE_INFORMATION:
			RYU_LOG_INFO("{}", formattedMsg);
			break;
			
		case asMSGTYPE_ERROR:
			RYU_LOG_ERROR("{}", formattedMsg);
			break;

		case asMSGTYPE_WARNING:
			RYU_LOG_WARN("{}", formattedMsg);
			break;
		}
	}
	...
	void ScriptEngine::ScriptLogMessageCallback(Logging::LogLevel level, const std::string& message)
	{
		...
		{
		case Trace:
			RYU_LOG_TRACE("AngelScript({}) - {}", callInfo, message);
			return;
		case Debug:
			RYU_LOG_DEBUG("AngelScript({}) - {}", callInfo, message);
			return;
		case Info:
			RYU_LOG_INFO("AngelScript({}) - {}", callInfo, message);
			return;
		case Warn:
			RYU_LOG_WARN("AngelScript({}) - {}", callInfo, message);
			return;
		case Error:
			RYU_LOG_ERROR("AngelScript({}) - {}", callInfo, message);
			return;
		case Fatal:
			RYU_LOG_FATAL("AngelScript({}) - {}", callInfo, message);
			return;
		}
	}
	...
	void ScriptEngine::LoadScriptFiles()
	{
		...
		{
			RYU_LOG_WARN("No script files (*.as) found in {}", m_scriptDir);
		}
